...
setPlayerState ::
     MonadState (player, players) m
  => (player -> ASetter' players s)
  -> s
  -> m ()
setPlayerState l = modifyPlayerState l . const

nextPlayer :: (MonadState (player, players) m, Cycling player) => m ()
nextPlayer = modify $ first successor

choosePlayer :: MonadState (player, players) m => player -> m ()
