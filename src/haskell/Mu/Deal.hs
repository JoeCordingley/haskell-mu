...
deal3WithUpdate ::
     (MonadRandom m) => (NOfThree -> [Card] -> m ()) -> m (Tuple3 [Card])
deal3WithUpdate update =
  shuffleAndDealWithUpdate update threePlayers reducedDeck

deal4 :: (MonadRandom m) => m (Tuple4 [Card])
deal4 = shuffleAndDeal fullDeck

deal4WithUpdate ::
     (MonadRandom m) => (NOfFour -> [Card] -> m ()) -> m (Tuple4 [Card])
...
deal5WithUpdate ::
     (MonadRandom m) => (NOfFive -> [Card] -> m ()) -> m (Tuple5 [Card])
...
deal6WithUpdate ::
     (MonadRandom m) => (NOfSix -> [Card] -> m ()) -> m (Tuple6 [Card])
...
shuffleAndDealWithUpdate ::
     (MonadRandom m, Traversable t)
  => (player -> [c] -> m ())
  -> t player
  -> [c]
  -> m (t [c])
shuffleAndDealWithUpdate update players =
  dealAndUpdate update players <=< shuffleM
...
dealAndUpdate ::
  ...
  -> [card]
  -> m (t [card])
dealAndUpdate update players = traverse update' . dealToPlayers players
  where
    update' (player, cards) = cards <$ update player cards

dealToPlayers :: Traversable t => t player -> [a] -> t (player, [a])
dealToPlayers players = evalState . deal' $ fmap emptyHand players
  where
    ...
    deal' hand = do
      ...
      if null remaining
        then return hand
        else dealRoundToPlayers hand >>= deal'

dealRoundToPlayers ::
     (MonadState [a] m, Traversable t) => t (player, [a]) -> m (t (player, [a]))
