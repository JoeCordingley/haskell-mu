...
import Servant ((:>), Capture, Proxy(..), Server, serve)
import Servant.API (FromHttpApiData, parseUrlPiece)
import Servant.API.WebSocket (WebSocket)
import Network.WebSockets       (Connection, sendTextData, receiveData, withPingThread, receiveDataMessage)
...
import Control.Concurrent (newMVar, MVar, modifyMVar, modifyMVar_, readMVar)
import Control.Lens hiding ((.=))
import           Data.Tuple.Homogenous
import Data.Text (Text)
import Control.Monad.State.Lazy (state, MonadState, runState)
...
import Control.Monad (forever, void, when, mzero, (<=<))
import GHC.Generics
import Data.Composition ((.*), (.**))
import Data.Aeson 
import qualified Data.HashMap.Strict as HM

newtype Name = Name String deriving (Eq, Ord, Generic)
...
newtype PlayerNumberInput = PlayerNumberInput String
...
server :: (MonadIO m, MonadError ServerError m) => MVar State -> Name -> Connection -> m ()
server stateRef name conn = keepAlive commmunication 
  where 
    keepAlive = liftEither <=< liftIO . withPingThread conn 30 (pure ()) . runExceptT 
    commmunication = do
      addConn'
      broadcastPlayers'
      ...
      wait conn
    broadcastPlayers' = sendJSON conn =<< CurrentPlayers . statePlayers <$> liftIO (readMVar stateRef) 
    addConn' = liftIO $ modifyMVar_ stateRef $ pure . over stateUserLens (Map.insert name conn)
    updatePlayer' n name = liftIO $ modifyMVar stateRef $ pure . updatePlayer n name
    broadcastUpdateToUser t n conn = sendJSON conn (NewPlayer n name) *> when (isJust $ sequence t) (sendJSON conn StartMsg)
    broadcastUpdateToUsers n (State users t) = traverse_ (broadcastUpdateToUser t n) users

data StartMsg = StartMsg deriving Generic
...
instance ToJSON NewPlayer where
  toJSON (NewPlayer number name) = object ["newPlayer" .= object ["number" .= number, "name" .= name]]

receiveJSON :: (MonadIO m, MonadError ServerError m, FromJSON a) => Connection -> m a
...
playerNumber' t = case T.unpack t of
  "1" -> return OneOfThree
  "2" -> return TwoOfThree
...
data PlayerNumberRequest = PlayerNumberRequest 
  { playerNumber:: NOfThree } deriving Generic
...
updatePlayer n = updated . updatePlayer' where
...
data CurrentPlayers = CurrentPlayers Players

instance ToJSON CurrentPlayers where
  toJSON (CurrentPlayers (Tuple3 (playerOne, playerTwo, playerThree))) = object ["currentPlayers" .= object ["playerOne" .= playerOne, "playerTwo" .= playerTwo, "playerThree" .= playerThree]]
...
statePlayersLens f s = fmap setPlayers . f $ statePlayers s where
  setPlayers u = s { statePlayers = u}

stateUserLens :: Lens' State (Map Name Connection)
stateUserLens f s = fmap setUsers . f $ stateUsers s where
...
data NOfThree = OneOfThree | TwoOfThree | ThreeOfThree 
...
broadcastPlayer :: (MonadIO m, Foldable f) => NOfThree -> Name -> f Connection -> m ()
broadcastPlayer n (Name name) = broadcast $ T.pack (show n <> ", " <> name)

broadcastPlayers :: (MonadIO m, Foldable f) => Tuple3 Name -> f Connection -> m ()
