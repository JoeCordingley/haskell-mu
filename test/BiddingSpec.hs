...
biddingProperties =
  testGroup
    ...
    , playEndsAfterFullSequenceOfPasses
    , playersGivenCorrectMaximum
    , playersRequestFromCardsLeft
    , biddingReturnsRaisesInOrder
    , biddingReturnsTheCardsLeftInHand
    ]
...
playEndsAfterFullSequenceOfPasses =
  testProperty "play ends after first full sequence of passes" $
  forAll biddingBids playEndsAfterFullSequenceOfPasses'
  where
    playEndsAfterFullSequenceOfPasses' (numberOfPlayers, bids) =
      indexAtLastPass == Just (length bids)
      where
        indexAtLastPass = indexAtLastPass' 1 0 bids
        indexAtLastPass' _ _ [] = Nothing
        indexAtLastPass' index _ (Raise _:rest) =
          indexAtLastPass' (index + 1) 0 rest
        indexAtLastPass' index passes (Pass:rest) =
          if passes + 1 == numberOfPlayers
            then Just index
            else indexAtLastPass' (index + 1) (passes + 1) rest

playersGivenCorrectMaximum :: TestTree
playersGivenCorrectMaximum =
  testProperty
    "players given a maximum that would take them one ahead of current max" $
  forAll maxBidGen allMaxesAreCorrect
  where
    allMaxesAreCorrect = all maxIsCorrect
    maxIsCorrect (maxBid, topTotal, playerTotal) =
      playerTotal + maxBid == topTotal + 1

playersRequestFromCardsLeft :: TestTree
playersRequestFromCardsLeft =
  testProperty "players asked to choose from the cards left in their hand" $
  forAll cardsLeftGen cardRequestsAllCorrect
  where
    cardRequestsAllCorrect (initialHands, requests) =
      all (requestIsCorrect initialHands) requests
    requestIsCorrect initialHands (player, cardsRequested, cardsBidSoFar) =
      cardsRequested `containsTheSameElementsAs`
      (findOrEmptyList player initialHands `minus` cardsBidSoFar)

biddingReturnsRaisesInOrder :: TestTree
biddingReturnsRaisesInOrder =
  testProperty "bidding returns the player raises in the order they occurred" $
  forAll returnedRaisesGen raisesRecorded
  where
    raisesRecorded (raises, playerBids) = raisesOf playerBids == raises

raisesOf :: [(Player, Bid)] -> [(Player, [Card])]
raisesOf = catMaybes . map playerRaise
  where
...
biddingReturnsTheCardsLeftInHand =
  testProperty "bidding returns the cards left in hand for each player" $
  forAll returnedRaisesAndInitialHands cardsInHandIsCorrect
  where
    cardsInHandIsCorrect (initialHands, cardsLeftInHand, playerBids) =
      (initialHands `minusMap` playerBids) `containsSameElementsAndKeysAs`
      cardsLeftInHand
...
containsSameElementsAndKeysAs ::
     (Ord k, Ord v) => Map k [v] -> Map k [v] -> Bool
containsSameElementsAndKeysAs m n = sortedAndSorted m == sortedAndSorted n
  where
...
biddingPlayers = do
  ...
  playersAskedInTurn <-
    execWriterT . mapWritten (fmap recordedPlayer) . unrwt $
...
biddingBids = do
  initialHands <- startingGen
  bids <-
    execWriterT . mapWritten (fmap recordedBid) . unrwt $
    runBidding initialHands
  return (length initialHands, bids)

biddingRecords :: Gen [Record]
biddingRecords = startingGen >>= execWriterT . unrwt . runBidding

returnedRaisesGen :: Gen ([(Player, [Card])], [(Player, Bid)])
returnedRaisesGen =
  startingGen >>=
  runWriterT .
  fmap finishedPlayerRaises . mapWritten (map playerBid) . unrwt . runBidding
  where
...
maxBidGen = totals <$> biddingRecords
  where
    totals records =
      zip3 (recordedMax <$> records) (topTotal records) (playerTotal records)
    topTotal =
      map (maximum . (0 :) . Map.elems) . runningTotals . (map playerBid)
    playerTotal = map (uncurry $ Map.findWithDefault 0) . playerAndRunningTotals
    playerAndRunningTotals records =
      zip (recordedPlayer <$> records) (runningTotals $ map playerBid records)
    playerBid r = (recordedPlayer r, recordedBid r)

runningTotalBids :: [(Player, Bid)] -> [Map Player [Card]]
runningTotalBids = scanl (flip accumulate) Map.empty
  where
...
cardsLeftGen ::
     Gen (Map Player [Card], [(Player, CardsRequested, CardsBidSoFar)])
cardsLeftGen = do
  initialHands <- startingGen
  requests <-
    execWriterT . mapWritten toRequests . unrwt $ runBidding initialHands
  return (Map.fromList initialHands, requests)
  where
    toRequests records =
      zip3
        (recordedPlayer <$> records)
        (recordedCards <$> records)
        (playerTotal records)
    playerTotal = map (uncurry $ findOrEmptyList) . playerAndRunningTotals
    playerAndRunningTotals records =
      zip
        (recordedPlayer <$> records)
        (runningTotalBids $ map playerBid records)
    playerBid r = (recordedPlayer r, recordedBid r)

returnedRaisesAndInitialHands ::
     Gen (InitialHandsMap, CardsLeftInHand, CardsBid)
returnedRaisesAndInitialHands = do
  initialHands <- startingGen
  (cardsLeftInHand, cardsBid) <-
    runWriterT . fmap finishedCardsInHand . mapWritten playerBids . unrwt $
    runBidding initialHands
...
newtype RecordWriterT a = RecordWriterT
  { unrwt :: WriterT [Record] Gen a
