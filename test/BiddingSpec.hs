...
biddingBids = do
  initialHands <- startingGen
  bids <- execWriterT . mapWritten (fmap recordedBid) $ runRecorded initialHands
  return (length initialHands, bids)

runRecorded ::
     InitialHands Player -> WriterT [Record] Gen (FinishedBidding Player)
...
returnedRaisesGen =
  startingGen >>=
  runWriterT .
  fmap finishedPlayerRaises . mapWritten (map playerBid) . runRecorded
  where
...
returnedRaisesAndInitialHands = do
  ...
  (cardsLeftInHand, cardsBid) <-
    runWriterT . fmap finishedCardsInHand . mapWritten playerBids $
    runRecorded initialHands
