...
module BiddingResolutionSpec (biddingResolutionTests, totalBidGenWithOutrightLeader) where
...
biddingResolutionTests = testGroup "Bidding resolution tests" [noRaisesReturnsEklatNoPoints]

noRaisesReturnsEklatNoPoints :: TestTree
noRaisesReturnsEklatNoPoints = testCase "No raises returns EklatNoPoints" $ (resolveBidding ([]::[(Player,[Card])]) @?= EklatNoPoints)
...
bidsWithMaxSize n players = Map.fromList <$> traverse pairWithGen players where
...
totalBidGenWithOutrightLeader = do
  ...
  totalBids <- Map.insert leader leadersBid <$> bidsWithMaxSize (leadersBidSize + 1) otherPlayers
...
splitToBids cardMap = do
  ...
  where
    playerBids player = map (player,) <$> (groupIntoNonEmptys $ findOrEmptyList player cardMap)
